...
path = "/home/schvarcz/Dropbox/Doutorado ENSTA/visualodometry/dataset_libviso/"
...
if __name__ == "__main__":
    ...
    for frameId in range(373):
        ...
        if oldCorrespondence != None:
            kptsLOld, descLOld= oldCorrespondence
            maxDisp = maxDispTensor
            matchesN2O = twoWayMatchAndLoweTest(flann, kptsL, descL, kptsLOld, descLOld, featureCorrespondenceCheck)
            matchesN2O, kptsLN, descLN, kptsLOld, descLOld, __ = cleanFeatures(matchesN2O, kptsL, descL, kptsLOld, descLOld)
            imgMN2O = drawFeaturesCorrespondance(imgl, kptsLN, kptsLOld, matchesN2O)

            kptsLN_np = kpts2np(kptsLN)
            kptsLOld_np = kpts2np(kptsLOld)

            fundMat, inliers = cv2.findFundamentalMat(kptsLN_np, kptsLOld_np)
            matchesI, kptsLI, kptsLOldI = cleanInliersFundamentalMat(inliers, matchesN2O, kptsLN, kptsLOld)
        ...
        if oldCorrespondence == None or flag:
            oldCorrespondence = (kptsL, descL)

        #Draw everything
        imgl = cv2.drawKeypoints(imgl,kptsL, None)

        drawPyPlotFeatures(imgl, imgl)
        if not imgMN2O is None:
            drawPyPlotFeatures(imgl, imgMN2O)
